        ...
        if (!empty($_SESSION['step3']) && is_file(PH7_ROOT_PUBLIC . '_constants.php')) {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['config_system_submit'])) {
                if (filled_out($_POST)) {
                    ...
                    $_SESSION['val']['phone_id'] = trim($_POST['phone_id']);
                    ...
                    if (validate_email($_SESSION['val']['bug_report_email'])) {
                      ...
                      if($_SESSION['val']['phone_id'] = "US") {
                            $sConfigContent = str_replace('%phone_id%', "/(\+|00)[0-9]{1,3}[0-9]{5,14}(x.+)?/", $sConfigContent);
                      }elseif ($_SESSION['val']['phone_id'] = "UK") {
                            $sConfigContent = str_replace('%phone_id%', "/^[0][1-9]\d{1}$|^[0]\d{10}$/", $sConfigContent);
                      }elseif ($_SESSION['val']['phone_id'] = "FR") {
                            $sConfigContent = str_replace('%phone_id%', "/^[0][1-9]\d{1}$|^[0]\d{9}$/", $sConfigContent);
                      }elseif ($_SESSION['val']['phone_id'] = "EN") {
                            $sConfigContent = str_replace('%phone_id%', "/^\d{9}$/", $sConfigContent);
                      }
