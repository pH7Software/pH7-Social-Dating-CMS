...
class CommentCoreModel extends Model
{
    ...
    public function gets($sTable, $iApproved = 1, $sOrder = self::UPDATED, $iOffset = 0, $iLimit = 500)
    {
        ...
        $rStmt = Db::getInstance()->prepare('SELECT c.*, m.username, m.firstName, m.sex FROM' . Db::prefix('Comments' . $sTable) . ' AS c LEFT JOIN' . Db::prefix(DbTableName::MEMBER) . 'AS m ON c.sender = m.profileId WHERE c.approved = :approved ORDER BY ' . $sOrder . ' DESC LIMIT :offset, :limit');
    ...
    public function read($iRecipientId, $iApproved, $iOffset, $iLimit, $sTable)
    {
        ...
        $rStmt = Db::getInstance()->prepare('SELECT c.*, m.username, m.firstName, m.sex FROM' .
            Db::prefix('Comments' . $sTable) . ' AS c LEFT JOIN' . Db::prefix(DbTableName::MEMBER) .
