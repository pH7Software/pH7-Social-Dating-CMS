...
class AvatarDesignCore extends Framework\Layout\Html\Design
{
     ...
     * @param integer $iApproved Show only approve avatar. Default 1
     */
    public function get($sUsername = '', $sFirstName = '', $sSex = null, $iSize = 32, $bRollover = false, $iApproved = 1)
    {
        ...
        if ($bRollover == 'rollover')
        {
            ...
            echo '<div class="rollover"><a href="', $this->_oUser->getProfileSignupLink($sUsername, $sFirstName, $sSex), '"><img src="', $this->getUserAvatar($sUsername, $sSex, $iSize, $iApproved), '" alt="', ucfirst($sUsername), '" title="', ucfirst($sFirstName), '" /></a></div>';
        }
        else
        {
            echo '<a class="pic" href="', $this->_oUser->getProfileSignupLink($sUsername, $sFirstName, $sSex), '"><img src="', $this->getUserAvatar($sUsername, $sSex, $iSize, $iApproved), '" alt="', ucfirst($sUsername), '" title="', ucfirst($sFirstName), '" class="avatar" /></a>';
        }
    }
     ...
     * @param integer $iApproved Show only approve avatar. Default 1
     */
    public function lightbox($sUsername = '', $sFirstName = '', $sSex = null, $iSize = 400, $iApproved = 1)
    {
        // The profile does not exist, so it creates a fake profile = ghost
        if (empty($sUsername))
        {
        ...
        echo '<div class="picture_block"><a href="', $this->getUserAvatar($sUsername, $sSex, $iSize, $iApproved), '" title="', ucfirst($sUsername), '" data-popup="image"><img src="', $this->getUserAvatar($sUsername, $sSex, $iSize / 2, $iApproved), '" alt="', ucfirst($sUsername), '" title="', ucfirst($sFirstName), '" class="img_picture" /></a></div>';
    }
