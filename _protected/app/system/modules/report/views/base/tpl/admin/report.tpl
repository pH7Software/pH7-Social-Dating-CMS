<div class="center">
  ...
  <p><span class="bold">{lang 'Reporter:'}</span> {{ $avatarDesign->get($oUserModel->getUsername($report->reporterId), $oUserModel->getFirstName($report->reporterId) ,null, 64) }}</p>
  <p><span class="bold">{lang 'Spammer:'}</span> {{ $avatarDesign->get($oUserModel->getUsername($report->spammerId), $oUserModel->getFirstName($report->spammerId) ,null, 64) }}</p>
