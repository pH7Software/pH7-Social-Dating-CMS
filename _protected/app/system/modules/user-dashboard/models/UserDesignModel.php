 ...
 * @package        PH7 / App / System / Module / User Dashboard / Model
 */
namespace PH7;
...
class UserDesignModel extends UserDesignCoreModel
{
    ...
    public function profilesBlock($iOffset = 0, $iLimit = self::PROFILE_BLOCK_LIMIT)
    {
        ...
        foreach ($oUser as $oRow)
        {
            ...
            echo '<li><a rel="nofollow" href="', $this->oUser->getProfileSignupLink($oRow->username, $sFirstName, $oRow->sex), '"><img src="', $this->getUserAvatar($oRow->username, $oRow->sex, $iSize, 'Members'), '" width="' . $iSize . '" height="' . $iSize . '" alt="',t('Meet %0% on %site_name%', $oRow->username), '" /></a></li>';
        }
