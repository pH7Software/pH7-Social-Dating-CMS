<?php
/**
 * @title          Video Form Processing Class
...
class VideoFormProcessing extends Form
{
    ...
    public function __construct()
    {
        ...
        if($this->httpRequest->postExists('embed_code'))
        {
            ...
            $sTitle = ($this->httpRequest->postExists('title') ? $this->httpRequest->post('title') : ($oInfo->getTitle() ? $oInfo->getTitle() : t('Untitled')));
        ...
        elseif(!empty($_FILES['video']['tmp_name']))
        {
            ...
            else
            {
                ...
                $sTitle = ($this->httpRequest->postExists('title')) ? $this->httpRequest->post('title') : ucfirst(str_replace(array('-', '_'), ' ', str_replace(PH7_DOT . $oVideo->getExt(), '', escape($_FILES['video']['name'], true))));
