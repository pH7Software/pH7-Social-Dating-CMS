...
class JoinForm
{
   ...
   public static function step1($iWidth = 300)
   {
        ...
        if (DbConfig::getSetting('isUniversalLogin'))
            $oForm->addElement(new \PFBC\Element\HTMLExternal('<div class="center"><a href="' . UriRoute::get('connect','main','index') . '" target="_blank" class="m_button">' . t('Universal Login') . '</a></div>'));
        ...
        $oForm->addElement(new \PFBC\Element\Checkbox(t('Terms of Service'), 'terms', array(1=>'<em>' . t('I have read and accept to the %0%.', '<a href="' . UriRoute::get('page','main','terms') . '" rel="nofollow" target="_blank">' . t('Terms of Service') . '</a>') . '</em>'), array('id'=>'terms', 'onblur'=>'CValid(this.checked, this.id)', 'required'=>1)));
    ...
    public static function step3()
    {
        ...
        $oForm->addElement(new \PFBC\Element\CKEditor(t('Description:'), 'description', array('id'=>'str_description', 'title'=>t('Describe yourself in a few words. Your description should be at least 20 characters long.'), 'onblur' =>'CValid(this.value,this.id,10,2000)', 'validation'=>new \PFBC\Validation\Str(10,2000), 'required' =>1)));
