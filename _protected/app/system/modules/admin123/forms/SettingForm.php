...
class SettingForm
{
    ...
    public static function display()
    {
        ...
        $oForm->addElement(new \PFBC\Element\Select(t('Protect for Users against session cookies hijacking:'), 'is_user_session_ip_check', ['1' => t('Yes  (recommend for security reasons)'), '0' => t('No')], ['description' => t('This protection can cause problems for logged in users with dynamic IPs. Please disable if so.'), 'value' => DbConfig::getSetting('isUserSessionIpCheck'), 'required' => 1]));

        $oForm->addElement(new \PFBC\Element\Select(t('Protect for Affiliates against session cookies hijacking:'), 'is_affiliate_session_ip_check', ['1' => t('Yes  (recommend for security reasons)'), '0' => t('No')], ['description' => t('This protection can cause problems for affiliates with dynamic IPs. Please disable if so.'), 'value' => DbConfig::getSetting('isAffiliateSessionIpCheck'), 'required' => 1]));

        $oForm->addElement(new \PFBC\Element\Select(t('Protect for Admins against session cookies hijacking:'), 'is_admin_session_ip_check', ['1' => t('Yes (highly recommend for security reasons)'), '0' => t('No')], ['description' => t('This protection can cause problems for admins with dynamic IPs. Please disable if so.'), 'value' => DbConfig::getSetting('isAdminSessionIpCheck'), 'required' => 1]));
