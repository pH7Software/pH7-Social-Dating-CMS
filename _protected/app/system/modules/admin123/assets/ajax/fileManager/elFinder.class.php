 ...
 **/
public
function commandArgsList($cmd)
{
    return $this->commandExists($cmd) ? $this->commands[$cmd] : [];
}

private
function session_expires()
{
...
function exec($cmd, $args)
{
    ...
    if (!empty($this->listeners[$cmd . '.pre'])) {
        foreach ($this->listeners[$cmd . '.pre'] as $handler) {
            call_user_func_array($handler, [$cmd, &$args, $this, $dstVolume]);
        }
    }
 ...
 */
protected
function saveNetVolumes($volumes)
{
 ...
 */
protected
function removeNetVolume($key, $volume)
{
    $netVolumes = $this->getNetVolumes();
    $res = true;
 ...
 */
protected
function getPluginInstance($name, $opts = [])
{
    $key = strtolower($name);
    if (!isset($this->plugins[$key])) {
        ...
        if (!class_exists($class)) {
            $p_file = dirname(
                    __FILE__
                ) . DIRECTORY_SEPARATOR . 'plugins' . DIRECTORY_SEPARATOR . $name . DIRECTORY_SEPARATOR . 'plugin.php';
            if (is_file($p_file)) {
                include_once $p_file;
            }
        }
...
function zipdl($args)
{
    ...
    if (!$download) {
        ...
        if (strpos($filenameEncoded, '%') === false) { // ASCII only
            ...
            } elseif (strpos($ua, 'Chrome') === false && strpos($ua, 'Safari') !== false && preg_match(
                    '#Version/[3-5]#',
                    $ua
                )) { // Safari < 6
 ...
 **/
protected
function get_remote_contents(&$url, $timeout = 30, $redirect_max = 5, $ua = 'Mozilla/5.0', $fp = null)
{
    $method = (function_exists('curl_exec') && !ini_get('safe_mode') && !ini_get(
            'open_basedir'
 ...
 **/
protected
function curl_get_contents(&$url, $timeout, $redirect_max, $ua, $outfp)
{
    $ch = curl_init();
 ...
 */
protected
function fsock_get_contents(&$url, $timeout, $redirect_max, $ua, $outfp)
{
    $connect_timeout = 3;
    $connect_try = 3;
    $method = 'GET';
 ...
 */
protected
function parse_data_scheme($str, $extTable)
{
    $data = $name = '';
 ...
 */
protected
function detectFileExtension($path)
{
    static $type, $finfo;
    if (!$type) {
 ...
 */
private
function getTempDir($volumeTempPath = null)
{
    $testDirs = [];
 ...
 */
private
function checkChunkedFile($tmpname, $chunk, $cid, $tempDir, $volume = null)
{
...
function upload($args)
{
    ...
    $shutdownfunc = function () {
        foreach (array_keys($GLOBALS['elFinderTempFiles']) as $file) {
 ...
 **/
protected
function getNetVolumeUniqueId($netVolumes = null, $prefix = 'nm')
{
    $id = false;
 ...
 **/
protected
function ensureDirsRecursively($volume, $target, $dirs, $path = '')
{
    $res = ['stats' => [], 'hashes' => []];
 ...
 */
public
static function isAnimationGif($path)
{
 ...
 */
public
static function isSeekableStream($resource)
{
 ...
 */
public
static function rewind($resource)
{
 ...
 */
public
static function sessionDataEncode($var)
{
 ...
 */
public
static function sessionDataDecode(&$var, $checkIs = null)
{
    if (self::$base64encodeSessionData) {
 ...
 */
public
static function getStaticVar($key)
{
 ...
 */
public
static function extendTimeLimit($time = null)
{
