    ...
    <form method="post">
        {each $sFolder in $oModule->showAvailableMods(Module::UNINSTALL)}
            ...
            {if $oModule->checkModFolder(Module::UNINSTALL, $sModsDirModFolder)}
                ...
                <button type="submit" class="btn btn-default btn-tiny" name="submit_mod_uninstall" value="{% $sModsDirModFolder %}" onclick="return confirm('{lang 'Are you sure you want to uninstall this module?'}');">{lang 'Uninstall'} {% $config->values['module.information']['name'] %}</button><br />
