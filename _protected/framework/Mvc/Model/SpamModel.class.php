...
use PH7\Framework\Mvc\Model\Engine\Db, PH7\Framework\Security\Spam\Spam;
...
class SpamModel
{
    ...
    public static function detectDuplicate($sCheckContent, $sFindColumn, $sColumnId, $iFindId, $sTable)
    {
        ...
        while ($oRow = $rStmt->fetch(\PDO::FETCH_OBJ))
            if ($bReturn = Spam::detectDuplicate($sCheckContent, $oRow->content)) break; // TRUE = Duplicate content detected, FALSE otherwise.
