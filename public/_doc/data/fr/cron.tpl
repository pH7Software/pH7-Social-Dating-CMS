...
<p>Par exemple pour exécuter la tâche de la base de données du serveur MySQL,<br />
 il suffit de l'exécuter (par exemple dans cPanel ou Plesk) toutes les 96 heures avec cette URL <pre>"GET http://VOTRE-SITE.COM/asset/ron/96h/Database/?secret_word=VOTRE_MOT_SECRET"</pre><br />
Vous avez aussi des paramètres GET "option" qui sont facultatifs. Exemple : <pre>"http://VOTRE-SITE.com/asset/ron/96h/Database/?secret_word=VOTRE_MOT_SECRET&option=repair"</pre> qui permet de réparer votre base de données dans le cas où votre base de données est détérioré ou encore réinitialiser les statistiques de votre site en passant comme paramètre "stat" à la place de "repair".
...
<p>Si vous avez des difficultés avec la configuration des Crons Jobs, veuillez simplement acheter un ticket sur notre site (dans la section soutien), et nous allons le configurer correctement pour vous.</p>

<p><em>Un soutien professionnel est la meilleure solution pour votre site internet !</em></p>
