...
needed: the `install geoip db` command of `_tools/pH7.sh` verifies or restores
it, and the [GeoIP database instructions](../_protected/framework/Geo/Ip/update-geo-database-version.txt)
explain why newer GeoLite data cannot be bundled. Preserve any newer database you
installed yourself when replacing application files.
