    ...
    {else}
        {each $user in $users}
            ...
            <div class="thumb_photo">
                ...
                <p class="cy_ico">
                    <a href="{{ $design->getProfileLink($user->username) }}" title="{lang 'First name: %0%', $user->firstName}<br> {lang 'Sex: %0% %1%', t($user->sex), $sex_ico}<br> {lang 'Seeking %0%', t($user->matchSex)}<br> {lang 'Age: %0%', $age}<br> {lang 'From %0%', $country_name}<br> {lang 'City %0%', $this->str->upperFirst($user->city)}<br> {lang 'State %0%', $this->str->upperFirst($user->state)}">
